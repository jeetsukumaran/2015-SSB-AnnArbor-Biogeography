...
#' @examples
#' test=1
section_the_tree <- function(inputs, make_master_table=FALSE, plot_pieces=TRUE, cut_fossils=TRUE, fossils_older_than=0.6, min_dist_between_node_and_stratum_line=0.00001, remove_root_edge=TRUE)
	{
	runjunk='
...
# 							print(temporary_fixlikes)
				...
				calc_loglike_sp_results = calc_loglike_sp(
					...
					states_allowed_TF=rep(TRUE, ncol(subtree_tip_relative_probs_of_each_state))
					)
...
# 					print(relative_probs_of_each_state_at_branch_bottom_below_node_DOWNPASS[master_tree_nodenums,][,states_allowed_TF])
					...
					for (uj in edges_to_visit_uppass)		# Since we are going backwards
						...
						if (hooknode_TF == TRUE)
							...
							if (include_null_range == TRUE)
								{
								num_clado_states = numstates - 1
								} else {
								num_clado_states = numstates
								} # if (include_null_range == TRUE)
				
							# Ancestral, left, and right states all the same
							temp_COO_weights_columnar[[1]] = 1:num_clado_states
							temp_COO_weights_columnar[[2]] = 1:num_clado_states
							temp_COO_weights_columnar[[3]] = 1:num_clado_states
							temp_COO_weights_columnar[[4]] = rep(1, num_clado_states)
							} else {
							temp_COO_weights_columnar = COO_weights_columnar
						...
						# 2015 version
						numstates = sum(states_allowed_TF)
						...
						if ((anc == rootnode) && (global_root_TF == TRUE))
							...
							# You are not at the global ancestor node
							if (anc == rootnode)
								{
								probs_at_mother = 1/length(starting_probs)
