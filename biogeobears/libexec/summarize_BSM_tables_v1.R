...
count_ana_clado_events <- function(clado_events_tables, ana_events_tables, areanames, actual_names)
	...
	all_totals_list = founder_totals_list + ana_totals_list + subsetSymp_totals_list + vicariance_totals_list + sympatry_totals_list
	...
	means = c(mean(founder_totals_list), mean(ana_totals_list), mean(a_totals_list), mean(d_totals_list), mean(e_totals_list), mean(subsetSymp_totals_list), mean(vicariance_totals_list), mean(sympatry_totals_list), mean(clado_totals_list), mean(all_totals_list))
	sds = c(sd(founder_totals_list), sd(ana_totals_list), sd(a_totals_list), sd(d_totals_list), sd(e_totals_list), sd(subsetSymp_totals_list), sd(vicariance_totals_list), sd(sympatry_totals_list), sd(clado_totals_list), sd(all_totals_list))
	sums = c(sum(founder_totals_list), sum(ana_totals_list), sum(a_totals_list), sum(d_totals_list), sum(e_totals_list), sum(subsetSymp_totals_list), sum(vicariance_totals_list), sum(sympatry_totals_list), sum(clado_totals_list), sum(all_totals_list))
	
	# Summarize directionality of dispersal
	founder_counts_fromto_means = adf2(apply(X=founder_counts_cube, MARGIN=c(1,2), FUN=mean))
	ana_counts_fromto_means = adf2(apply(X=ana_counts_cube, MARGIN=c(1,2), FUN=mean))
	...
	names(ana_counts_fromto_means) = actual_names
	row.names(ana_counts_fromto_means) = actual_names
	...
	names(ana_counts_fromto_sds) = actual_names
	row.names(ana_counts_fromto_sds) = actual_names
	...
	counts_list$ana_counts_fromto_means = ana_counts_fromto_means
	...
	counts_list$ana_counts_fromto_sds = ana_counts_fromto_sds
	...
	cat("\nmeans:\n")
	print(conditional_format_table(counts_list$ana_counts_fromto_means))
	cat("\nstandard deviations:\n")
	print(conditional_format_table(counts_list$ana_counts_fromto_sds))
	...
	names(summary_counts_BSMs) = c("founder", "ana_disp", "a", "d", "e", "subset", "vicariance", "sympatry", "all_clado", "total_events")
