...
stochastic_map_branch <- function(nodenum_at_top_of_branch, trtable, Qmat, state_indices_0based, ranges_list, areas, single_branch=FALSE, stratified=FALSE, maxtries=40000)
	{
	...
	# Stratified analysis have SUBedge.length columns, however these may not have
	...
	while (1)
		...
		repeat
			...
			# Don't allow more than 100 events on a branch (!)
		...
		if (trynum > maxtries)
			...
			errortxt = paste("\n\nError in stochastic simulation; no success on branch below node ", node_in_master_tree_w_error, " after ", maxtries, " tries. Printing starting/ending states, and events table for last attempt. I suggest using a try() statement when running this.\n\nReturning object of class 'try-error'.", sep="")
			cat(errortxt)
			...
			error_msg_for_stop = paste("Stochastic mapping failed after maxtries=", maxtries, " tries on branch below node=", nodenum_at_top_of_branch, sep="")
	...
	events_table_for_branch = as.data.frame(events_table_for_branch)
...
get_inputs_for_stochastic_mapping <- function(res, cluster_already_open=NULL, rootedge=FALSE, statenum_bottom_root_branch_1based=NULL, printlevel=1, min_branchlength=0.000001)
	{
	defaults='
	res=res
...
get_inputs_for_stochastic_mapping_from_results_object <- function(res, cluster_already_open=NULL, rootedge=FALSE, statenum_bottom_root_branch_1based=NULL, printlevel=1, stratified=FALSE, min_branchlength=0.000001, timeperiod_i=1)
	{
	defaults='
	cluster_already_open=NULL
	rootedge=FALSE
	...
	returned_mats2 = get_Qmat_COOmat_from_res(res, timeperiod_i=timeperiod_i)
	returned_mats2
	...
	return(stochastic_mapping_inputs)
	} # END get_inputs_for_stochastic_mapping_from_results_object <- function(res, cluster_already_open=NULL, rootedge=FALSE, statenum_bottom_root_branch_1based=NULL, printlevel=1, stratified=FALSE, min_branchlength=0.000001, timeperiod_i=1)
...
# to be run once.
#######################################################
get_inputs_for_stochastic_mapping_stratified <- function(res, cluster_already_open=NULL, rootedge=FALSE, statenum_bottom_root_branch_1based=NULL, printlevel=1, min_branchlength=0.000001)
	{
	defaults='
	res=res
	cluster_already_open=NULL
...
# 					}
				...
				events_table_for_branch = stochastic_map_branch(nodenum_at_top_of_branch=subtable_rownum, trtable=master_table_timeperiod_i, Qmat, state_indices_0based, ranges_list, areas, single_branch=single_branch, stratified=stratified, maxtries=maxtries)
				...
				# for branch tops
				master_table_timeperiod_i = stochastic_map_given_inputs(stochastic_mapping_inputs, piecenum=piecenum, maxtries=maxtries, include_null_range=res$inputs$include_null_range, master_nodenum_toPrint=master_nodenum_toPrint)
...
make_BSMs <- function(res1, model_name="DEC", BSM_tables_dir="BSM_tables_M3_v1", BSM_fn_base="BSM_M3_", num_maps=5, maxtries=1000000, groupname="", seedval=as.numeric(Sys.time()))
	...
	if (is.na(res1$inputs$timesfn) == TRUE)
		...
		stochastic_mapping_inputs = get_inputs_for_stochastic_mapping_from_results_object(res=res1)
		} else {
		stratified = TRUE
		res1$inputs$stratified = TRUE
		stochastic_mapping_inputs_list = get_inputs_for_stochastic_mapping_stratified(res=res1)
	...
	while( mapnum <= num_maps )
		...
		if (stratified == TRUE)
			...
			cmdstr = paste("stochastic_mapping_results = stochastic_mapping_on_stratified(res=res1, stochastic_mapping_inputs_list=stochastic_mapping_inputs_list, maxtries=", maxtries, ")", sep="")
			} else {
			# Non-stratified stochastic mapping
			cmdstr = paste("stochastic_mapping_results = stochastic_map_given_inputs(stochastic_mapping_inputs=stochastic_mapping_inputs, maxtries=", maxtries, ", include_null_range=include_null_range)", sep="")
			}
